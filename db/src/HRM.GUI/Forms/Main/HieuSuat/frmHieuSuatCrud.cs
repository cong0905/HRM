...
public sealed partial class frmHieuSuat
{
    ...
    private async Task LoadGridAsync()
    {
        ...
        try
        {
            ...
            // Cập nhật biểu đồ theo dữ liệu hiện tại
            UpdateCharts(data);
        }
        catch (Exception ex)
        {
    ...
    /// <summary>Cập nhật cả 3 biểu đồ dựa trên dữ liệu hiện tại của grid.</summary>
    private void UpdateCharts(List<HieuSuatDTO> data)
    {
        try
        {
            UpdateColumnChart(data);
            UpdatePieChart(data);
            UpdateLineChartAsync();
        }
        catch
        {
            // Không hiển thị lỗi chart để tránh làm phiền user
        }
    }

    /// <summary>Biểu đồ cột nhóm: KPI / Deadline / Chuyên cần cho từng nhân viên.</summary>
    private void UpdateColumnChart(List<HieuSuatDTO> data)
    {
        _columnChartData.Clear();

        // Lấy tối đa 15 nhân viên có dữ liệu, sắp xếp theo tổng điểm giảm dần
        var topItems = data
            .Where(x => !string.IsNullOrWhiteSpace(x.TenNhanVien))
            .OrderByDescending(x => x.DiemHieuSuatTong)
            .Take(15)
            .ToList();

        foreach (var item in topItems)
        {
            _columnChartData.Add(new ColumnChartItem
            {
                TenNhanVien = item.TenNhanVien ?? "?",
                DiemKPI = item.DiemKPI ?? 0,
                TyLeHoanThanhDeadline = item.TyLeHoanThanhDeadline ?? 0,
                DiemChuyenCan = item.DiemChuyenCan ?? 0
            });
        }

        _chartColumn.Invalidate();
    }

    /// <summary>Biểu đồ tròn (Doughnut): phân loại nhân viên theo DiemHieuSuatTong.</summary>
    private void UpdatePieChart(List<HieuSuatDTO> data)
    {
        if (data.Count == 0)
        {
            _pieChartTotal = 0;
            ...
            _pieKha = 0;
            _pieTb = 0;
            _chartPie.Invalidate();
            return;
        }

        _pieChartTotal = data.Count;
        _pieXuatSac = data.Count(x => x.DiemHieuSuatTong >= 90);
        _pieTot = data.Count(x => x.DiemHieuSuatTong >= 75 && x.DiemHieuSuatTong < 90);
        _pieKha = data.Count(x => x.DiemHieuSuatTong >= 60 && x.DiemHieuSuatTong < 75);
        _pieTb = data.Count(x => x.DiemHieuSuatTong < 60);

        _chartPie.Invalidate();
    }

    /// <summary>Biểu đồ đường: xu hướng điểm trung bình qua các kỳ đánh giá.</summary>
    private async void UpdateLineChartAsync()
    {
        try
        {
            _lineChartData.Clear();

            // Lấy toàn bộ dữ liệu để tính xu hướng qua các kỳ
            var allData = await _hieuSuatService.GetAllAsync();
            var kyList = await _hieuSuatService.GetKyDanhGiaAsync();

            if (allData.Count == 0 || kyList.Count == 0)
            {
                _chartLine.Invalidate();
                return;
            }

            // Group data theo kỳ, tính trung bình, sắp xếp theo ngày bắt đầu
            var kyDict = kyList.ToDictionary(k => k.MaKyDanhGia, k => k);
            var grouped = allData
                .Where(x => kyDict.ContainsKey(x.MaKyDanhGia))
                .GroupBy(x => x.MaKyDanhGia)
                .Select(g => new
                {
                    Ky = kyDict[g.Key],
                    DiemTB = g.Average(x => (double)x.DiemHieuSuatTong)
                })
                .OrderBy(x => x.Ky.NgayBatDau)
                .ToList();

            foreach (var item in grouped)
            {
                _lineChartData.Add(new LineChartItem
                {
                    TenKyDanhGia = item.Ky.TenKyDanhGia,
                    DiemTB = item.DiemTB
                });
            }
